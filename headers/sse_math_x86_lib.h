...
namespace sse_utility {
	...
	template<> 
	SSE_MATH_X86_API float* aligned_alloc<float>(std::size_t size, std::size_t alignment);
	// aligned allocation for double-precision floating point
	template<> 
	SSE_MATH_X86_API double* aligned_alloc<double>(std::size_t size, std::size_t alignment);
